...
function readData() {
  ...
  return { username: process.env.DEFAULT_USERNAME || 'admin', password: process.env.DEFAULT_PASSWORD || 'admin', links: [], notes: [] };
}
...
function extractMeta(html, baseUrl) {
  ...
  let m;
  while ((m = iconRe.exec(html)) !== null) candidates.push(m[1]);

  const shortcut = html.match(/<link[^>]+rel=["'][^"']*shortcut icon[^"']*["'][^>]+href=["']([^"']+)["']/i);
  if (shortcut) candidates.push(shortcut[1]);
  candidates.push('/favicon.ico');

  const seen = new Set();
  const favicons = [];
  for (const c of candidates) {
    try { const abs = new URL(c, base).href; if (!seen.has(abs)) { seen.add(abs); favicons.push(abs); } } catch {}
  }
...
async function checkFavicon(url) {
  try {
    ...
    if (r.status >= 200 && r.status < 400 && r.body.length > 0) return url;
  } catch {}
  return null;
}
...
// ── FAVICON PROXY (avoids mixed-content/CORS issues) ──
...
// ── PUBLIC DATA ──
app.get('/api/links', (req, res) => res.json(readData().links));
app.get('/api/notes', (req, res) => res.json(readData().notes));

// ── PROTECTED ──
app.post('/api/links', requireAuth, (req, res) => {
  const { name, url, icon, desc, faviconUrl } = req.body;
  if (!name || !url) return res.status(400).json({ error: 'name and url required' });
  const d = readData();
  d.links.push({ name, url, icon: icon || '', desc: desc || '', faviconUrl: faviconUrl || null });
  writeData(d);
  res.json({ ok: true, links: d.links });
});
...
app.delete('/api/links/:index', requireAuth, (req, res) => {
  ...
  d.links.splice(i, 1); writeData(d);
  res.json({ ok: true, links: d.links });
});
...
app.delete('/api/notes/:index', requireAuth, (req, res) => {
  ...
  d.notes.splice(i, 1); writeData(d);
  res.json({ ok: true, notes: d.notes });
});
